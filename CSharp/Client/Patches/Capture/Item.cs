...
namespace ShowPerfExtensions
{
  public partial class Mod : IAssemblyPlugin
  {
    ...
    {
      if (!Capture.ItemComponentsUpdate.IsActive) return true;
      Window.EnsureCategory(Capture.ItemComponentsUpdate.Category);
      ...
      if (Showperf.ShouldCapture(_))
      {
        ...
        {
          Window.AddTicks(new UpdateTicks(ticks, Capture.ItemComponentsUpdate.Category, $"{_}.AITarget"));
        }
        else
        {
          Window.AddTicks(new UpdateTicks(ticks, Capture.ItemComponentsUpdate.Category, $"{_.Prefab.Identifier.Value}.AITarget"));
        }
      }
      ...
      for (int i = 0; i < _.updateableComponents.Count; i++)
      {
        ...
        if (Showperf.ShouldCapture(_))
        {
          if (Capture.ItemComponentsUpdate.ByID)
          {
            Window.AddTicks(new UpdateTicks(ticks, Capture.ItemComponentsUpdate.Category, $"{_}.{ic.Name}"));
          }
          else
          {
            Window.AddTicks(new UpdateTicks(ticks, Capture.ItemComponentsUpdate.Category, $"{_.Prefab.Identifier.Value}.{ic.Name}"));
          }
        }
      }
